...
mq.addEventListener?.('change', (e) => {
  ...
  // Measure the fixed footer and expose --footer-h to CSS
...
window.getPresets = (t) => {
  ...
  // After rebuilding the form, (re)bind color↔hex pairs and seed values
  requestAnimationFrame(() => {
  // run the robust matcher declared at the bottom of the file
  window.bindColorHexSync?.();

  // also seed any visible hex fields next to color chips with the chip's value
  details.querySelectorAll('input[type="color"]').forEach(col => {
    const text = color.parentElement?.querySelector('input[type="text"]')
          || color.closest('label')?.querySelector('input[type="text"]');
    if (!text) return;
    const hx = (col.value || '').toUpperCase();
    text.value = hx;            // actual value (overrides any “RRGGBB”)
    text.placeholder = hx;      // so placeholder also reflects the real color
  });
});

  // seed hex fields next to color inputs
  details.querySelectorAll('input[type="color"]').forEach(col => {
    const text = col.closest('label')?.querySelector('input[type="text"]')
              || col.parentElement?.querySelector?.('input[type="text"]');
    if (text) text.value = (col.value || '').toUpperCase();
  });

  window.reflowStepper && window.reflowStepper();
  window.bindColorHexSync?.();   // ensure color–hex fields sync after rebuild
  }
...
typeSel.addEventListener('change', () => {
  ...
  // Rebuild the fields FIRST
  renderTypeForm(t);

  if (!currentPresetIdx.has(t)) currentPresetIdx.set(t, 0);
  applyPreset(t, currentPresetIdx.get(t));   // this already sets caption via preset
...
if (initialType && getPresets(initialType).length) {
  ...
  if (msgMode && msgMode.value === 'Resistbot') applyResistbot();

  // Initial render only — form rebuild happens inside main type change listener
...
(function () {
  ...
  function buildText(){
    ...
    switch(t){
      case "URL": {
        const u = val("urlData") || document.getElementById("urlData")?.getAttribute("placeholder") || "https://example.org";
        return u;
      }
      case "Payment": {
...
// ===== ECC state (session-persistent) =====
const ECC_KEY = 'okqral_ecc';          // sessionStorage key
const ECC_DEFAULT = 'M';

function getECC(){
  // session-scoped; change to localStorage if you want across tabs/restarts
...
function setECC(val, {trigger=true} = {}){
  ...
  // reflect to the pill UI if present
  ...
  // legacy: if a hidden <select id="ecc"> ever exists, keep it in sync
  ...
  // (optional) analytics
  try { sendEvent && sendEvent('ecc_change', { ecc: v, ...currentUiState() }); } catch {}
}
...
function wireECCPill(){
  ...
  // 1) Event delegation (one listener handles all buttons)
  pill.addEventListener('click', (e) => {
    const btn = e.target.closest('.ecc-btn');   // ← matches your HTML
    if (!btn || !pill.contains(btn)) return;

    const v = (btn.dataset.ecc || '').toUpperCase();  // 'L' | 'M' | 'Q' | 'H'
    if (!v) return;

    if (ecc) {
      ecc.value = v;
      ecc.dispatchEvent(new Event('change', { bubbles: true }));
    } else {
      setECC(v);
  }
});

  // 2) Keep pill state in sync when the <select> changes
  function syncFromSelect(){
    const v = (ecc.value || '').toUpperCase();
    for (const b of pill.querySelectorAll('.ecc-btn')) {
      b.setAttribute('aria-pressed', b.dataset.ecc === v ? 'true' : 'false');
    }
  }

  // 3) Wire and initialize
  ecc.addEventListener('change', syncFromSelect);
  syncFromSelect();
}

// Add .mech-active to #stepper when Mechanicals card is open (for CSS to show the pill)
function watchMechanicalsOpen(){
  const stepper = document.getElementById('stepper');
  if (!stepper) return;
  const mech = stepper.querySelector('.step-card[data-step="mechanicals"]');
  if (!mech) return;

  const refresh = () =>
    stepper.classList.toggle('mech-active', mech.classList.contains('is-open'));

  refresh(); // initial
  new MutationObserver(refresh).observe(mech, { attributes:true, attributeFilter:['class']});
}
...
window.addEventListener('resize', () => requestAnimationFrame(setFooterVar));
...
if (document.readyState === 'loading') {
  ...
  // create viewport that respects footer
...
function refreshModulesMode(){
  ...
  const shapeSel  =
    ...
    document.getElementById('moduleShape') ||   // ← add this id
  ...
  const bodyHex =
    document.getElementById('bodyColorHex') ||
...
(function bindColorHexSync(){
  const ready = () => {
    ...
    // Known explicit pairs (ID ↔ ID)
    const EXPLICIT = [
      ['captionColor',   'captionColorHex'],
      ['bodyColor',      'bodyColorHex'],
      ['eyeRingColor',   'eyeRingColorHex'],
      ['eyeCenterColor', 'eyeCenterColorHex'],
      ['bgColor',        'bgColorHex'],
    ];

    // 1) Wire explicit pairs first
    EXPLICIT.forEach(([colorId, textId]) => {
      const colorEl = document.getElementById(colorId);
      const textEl  = document.getElementById(textId);
      if (!colorEl || !textEl) return;

      // seed
      textEl.value = (toHex(textEl.value) || colorEl.value || '').toUpperCase();
      ...
      // Hex → Color (validate on input/change/blur)
      const applyHex = () => {
      ...
      colorEl.dataset.hexWired = '1'; // avoid auto-match
    });

    // 2) Auto-match any remaining color inputs
    document.querySelectorAll('input[type="color"]').forEach(colorEl => {
      if (colorEl.dataset.hexWired === '1') return;

      const wrapper =
        colorEl.closest('.color-field, .field, .control, .form-row, .flex, .grid, div') ||
        colorEl.parentElement;
