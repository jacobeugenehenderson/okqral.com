...
const cardH = (showCaption ? totalH : size) - inset * 2;
...
// Card background / frame now lives on the PREVIEW CONTAINER
const previewEl = document.getElementById('qrPreview');
const wantTransparent = !!document.getElementById('bgTransparent')?.checked;

// use the chosen BG color for either the fill or the stroke
const fillColor   = colorHex('bgColor', '#FFFFFF');
// or, if you prefer a different frame color, swap this:
// const frameColor = colorHex('eyeRingColor', '#9CA3AF');
const frameColor  = fillColor;

// keep the rounded corners that Tailwind sets (rounded-xl)
previewEl.style.borderRadius = getComputedStyle(previewEl).borderRadius;

// compute a sensible visual stroke width based on container width
const pw = previewEl.clientWidth || 320;          // pixels
const framePx = Math.max(.125, Math.round(pw * 0.00075)); // ~1.2% of width

if (wantTransparent) {
  // show decorative stroke, no fill
  previewEl.style.background = 'transparent';
  previewEl.style.border = `${framePx}px solid ${frameColor}`;
} else {
  // show solid fill, no stroke
  previewEl.style.background = fillColor;
  ...
  const SCALE   = showCap ? 0.70 : 0.66;            // a touch more buffer when no caption
  ...
  try {
    svg = buildQrSvg({
      ...
      transparentBg: !!document.getElementById('bgTransparent')?.checked,
