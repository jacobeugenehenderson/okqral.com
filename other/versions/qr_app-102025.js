...
function render() {
  ...
  const svg = composeCardSvg({
    ...
    captionText:    document.getElementById('campaign')?.value || '',
...
(function wireStepper() {
  ...
  // Re-apply sizing for the currently open panel
  window.reflowStepper = function () {
    const root   = document.getElementById('stepper');
    if (!root) return;
    const panels = Array.from(root.querySelectorAll('[data-step-panel]'));
